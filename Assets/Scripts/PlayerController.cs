...
public class PlayerController : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetAxis("Vertical_P1") < 0 && !_controller.isGrounded)
        {
        ...
        if (doubleJump && Input.GetButtonDown("Jump_P1"))
        {
            velocity.y = 0;
            velocity.y = Mathf.Sqrt(2f * jumpHeight * -gravity);
            doubleJump = false;
        }
        else if (Input.GetButtonDown("Jump_P1") && _controller.isGrounded)
        {
            velocity.y = Mathf.Sqrt(2f * jumpHeight * -gravity);
            doubleJump = true;
        }
